...
const DEFAULT_TREE = {
  id: 'root', name: 'Root Node', color: '#3b82f6', collapsed: false,
  children: [
    { id: 'node-1', name: 'Frontend', color: '#10b981', collapsed: false, children: [
    ...
    { id: 'node-2', name: 'Backend', color: '#f59e0b', collapsed: false, children: [
    ...
    { id: 'node-3', name: 'DevOps', color: '#8b5cf6', collapsed: false, children: [
...
function loadFromStorage() {
  try { const s = localStorage.getItem('pr-task-tree'); return s ? JSON.parse(s) : JSON.parse(JSON.stringify(DEFAULT_TREE)) }
...
function generateId() { return 'node-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7) }
...
function findParent(node, id) {
  if (node.children) { for (const c of node.children) { if (c.id === id) return node; const f = findParent(c, id); if (f) return f } }
  return null
}
...
export const useTreeStore = defineStore('tree', () => {
  ...
  function addChild(parentId) {
    ...
    if (parent) {
      ...
      parent.children.push({ id: generateId(), name: 'New Node', color: parent.color, collapsed: false, children: [] })
  ...
  function deleteNode(nodeId) {
    ...
    if (parent) { parent.children = parent.children.filter(c => c.id !== nodeId); save() }
  }

  function resetTree() { tree.value = JSON.parse(JSON.stringify(DEFAULT_TREE)); save() }

  function exportJSON() {
    const data = { version: '1.0', name: 'Tree Diagram Export', createdAt: new Date().toISOString(), tree: JSON.parse(JSON.stringify(tree.value)) }
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
    const url  = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `tree-diagram-${Date.now()}.json`; a.click()
  ...
  function importJSON(jsonData) {
    try {
      ...
      tree.value = data.tree || (data.id ? data : (() => { throw new Error('Invalid format') })())
      save()
      return { ok: true }
  ...
  async function saveToSheets() {
    if (!auth.sheetId || !auth.isOAuthReady) {
      ...
      sheetsMsg.value    = !auth.isOAuthReady ? 'Belum login Google' : 'Sheet ID belum diisi'
  ...
  async function loadFromSheets() {
    ...
    try {
      ...
      if (loaded) { tree.value = loaded; save(); sheetsStatus.value = 'ok'; sheetsMsg.value = '✅ Tree dimuat dari Google Sheets'; return { ok: true } }
      else { sheetsStatus.value = 'ok'; sheetsMsg.value = 'Sheet kosong, tree tidak berubah'; return { ok: true, empty: true } }
    } catch (e) {
      sheetsStatus.value = 'error'
  ...
  return { tree, sheetsStatus, sheetsMsg, toggleCollapse, updateNodeColor, updateNodeName, addChild, deleteNode, resetTree, exportJSON, importJSON, saveToSheets, loadFromSheets }
})
